...
static void SYSTEM_ExtendHeap (LONGINT blksz)
{
	...
	if (chnk != 0) {
		...
		if (next == 0) {
			__GET(chnk + 4, SYSTEM_heapend, LONGINT);
		}
	}
}
...
SYSTEM_PTR SYSTEM_NEWREC (LONGINT tag)
{
	...
	if (i < 9) {
		...
		for (;;) {
			...
			__GET(adr + 4, t, LONGINT);
			if (t >= blksz) {
				break;
			}
			prev = adr;
			__GET(adr + 12, adr, LONGINT);
		}
		restsize = t - blksz;
		end = adr + restsize;
		__PUT(end + 4, blksz, LONGINT);
...
SYSTEM_PTR SYSTEM_NEWBLK (LONGINT size)
{
	...
	__PUT((LONGINT)new - 4, tag, LONGINT);
	SYSTEM_Unlock();
	return new;
}
			...
			if (__ODD(tag)) {
				if (freesize > 0) {
					...
					if (i < 9) {
						...
						__PUT(start + 12, SYSTEM_bigBlocks, LONGINT);
...
static void SYSTEM_InitHeap (void)
{
	SYSTEM_heap = SYSTEM_NewChunk(128000);
	__GET(SYSTEM_heap + 4, SYSTEM_heapend, LONGINT);
...
__TDESC(SYSTEM_ModuleDesc__desc, 1, 2) = {__TDFLDS("ModuleDesc", 48), {0, 28, -12}};
__TDESC(SYSTEM_CmdDesc__desc, 1, 1) = {__TDFLDS("CmdDesc", 32), {0, -8}};
__TDESC(SYSTEM_FinDesc__desc, 1, 1) = {__TDFLDS("FinDesc", 16), {0, -8}};
__TDESC(SYSTEM__1__desc, 1, 1) = {__TDFLDS("", 8), {4, -8}};
