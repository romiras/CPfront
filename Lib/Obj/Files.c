...
static void Files_Create (Files_File f)
{
	...
	if (f->fd == -1) {
		...
		f->fd = Unix_Open(f->workName, 101, 0x8302, 0x01b4);
		done = f->fd >= 0;
		errno = Unix_errno();
...
Files_File Files_Old (CHAR *name, LONGINT name__len)
{
	...
	if (name[0] != 0x00) {
		...
		for (;;) {
			fd = Unix_Open(path, 256, 0x8002, 0x0);
			done = fd >= 0;
			errno = Unix_errno();
			...
			if (!done && ((errno == 13 || errno == 30) || errno == 11)) {
				fd = Unix_Open(path, 256, 0x8000, 0x0);
				done = fd >= 0;
				errno = Unix_errno();
			}
			if (!done && errno != 2) {
...
void Files_Rename (CHAR *old, LONGINT old__len, CHAR *new, LONGINT new__len, INTEGER *res)
{
	...
	if (r >= 0) {
		...
		if (r < 0) {
			*res = (int)Unix_errno();
			if (*res == 18) {
				fdold = Unix_Open(old, old__len, 0x8000, 0x0);
				if (fdold < 0) {
					*res = 2;
					__DEL(old);
					__DEL(new);
				...
				fdnew = Unix_Open(new, new__len, 0x8302, 0x01b4);
				if (fdnew < 0) {
					r = Unix_Close(fdold);
					*res = 3;
					__DEL(old);
