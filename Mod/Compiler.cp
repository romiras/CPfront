MODULE OfrontCompiler;
	...
	IMPORT Kernel,
		...
		DevCPM, DevCPT, DevCPB, DevCPP, DevCPV := OfrontCPV, DevCPG := OfrontCPG;

	CONST
		(* compiler options: *)
		checks = 0; allchecks = 1; assert = 2; obj = 3; ref = 4; allref = 5; srcpos = 6;
		mainprog = 20; include0 = 21; errorTrap = 31;
	...
	BEGIN
		...
		IF mainprog IN opt THEN INCL(DevCPG.opt, DevCPG.mainprog) END;
		IF include0 IN opt THEN INCL(DevCPG.opt, DevCPG.include0) END;
		DevCPT.Init(opt);
(*
		ASSERT(DevCPT.char16typ.size = 2);
		DevCPT.char16typ.size := 4;
*)
		DevCPB.typSize := NIL; (* DevCPV.TypSize *)
		DevCPT.processor := DevCPV.processor;
		DevCPP.Module(p);
		IF DevCPM.noerr THEN
			IF DevCPT.libName # "" THEN EXCL(opt, obj) END;
			DevCPV.Init(opt); DevCPV.AdrAndSize(DevCPT.topScope); DevCPT.Export(ext, new);
		...
		IF DevCPM.noerr & (DevCPG.mainprog IN DevCPG.opt) & (DevCPG.modName # "SYSTEM") THEN
			DevCPM.DeleteNewSym; DevCPM.LogWStr("  main program")
...
*)
	...
	BEGIN
		...
		DevCPG.OpenPar;
		(* DevCPG.Init(done); *) done := TRUE;
		error := ~done;
		IF ~error THEN Module(sourceR, opt, log, error) END
	END Do;
	...
	BEGIN
		...
		WHILE opt[i] # 0X DO
			...
			ELSIF opt[i] = "$" THEN INCL(opts, mainprog)
			ELSIF opt[i] = "#" THEN INCL(opts, include0)
			END;
			INC(i)
		END;
		Open;
		...
		IF one & (s.start >= end) THEN
			...
			WHILE (s.start < end) & (s.type = TextMappers.string) & ~error DO
				...
				WHILE (s.start < end) & (s.type = TextMappers.char) DO
					...
					ELSIF s.char = "$" THEN INCL(opts, mainprog)
					ELSIF s.char = "#" THEN INCL(opts, include0)
					ELSIF s.char = "(" THEN
						s.Scan;
